...
try {
    if ($type == "Insert") {
        ...
        $res = $pdo->prepare("INSERT INTO avaliacoes_ponto_turisticos (comentario_avaliacao_pt, qtd_estrela_avaliacao_pt, id_ponto_turistico, id_usuario, data_avaliacao) 
                              VALUES (:comentario_avaliacao_pt, :qtd_estrela_avaliacao_pt, :id_ponto_turistico, :id_usuario, :data_avaliacao)");
        ...
        $res = $pdo->prepare("UPDATE avaliacoes_ponto_turisticos 
                              SET comentario_avaliacao_pt = :comentario_avaliacao_pt, qtd_estrela_avaliacao_pt = :qtd_estrela_avaliacao_pt, data_avaliacao = :data_avaliacao 
                              WHERE id_avaliacao_pt = :id_avaliacao_pt");
        $res->bindValue(":comentario_avaliacao_pt", $comment);
        $res->bindValue(":qtd_estrela_avaliacao_pt", $stars);
        $res->bindValue(":data_avaliacao", $date);
